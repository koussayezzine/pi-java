...
public class SupprimerTicket {
    ...
    private void deleteTicket() {
        ...
        try {
            // Try to load the CSS from different possible locations
            URL cssUrl = getClass().getResource("/tn/esprit/sirine/styles.css");
            if (cssUrl == null) {
                cssUrl = getClass().getResource("/style/styles.css");
            }
            if (cssUrl == null) {
            ...
            if (cssUrl != null) {
                ...
                System.err.println("Warning: Could not find CSS file in any location");
                // Continue without CSS - the dialog will use default styling
            }
        } catch (Exception e) {
    ...
    private void updateTicketDetails(Ticket ticket) {
        ...
        if (adjustedQrPath != null && qrFile != null && qrFile.exists() && qrFile.isFile()) {
